(() => {
  ...
  function render() {
    ...
    if(target.campaign){dayIndex=Math.min(dayIndex,target.campaign.days.length-1);const day=target.campaign.days[dayIndex],dayStars=day.starTargets||stars;fillSelect($('admin-campaign-day'),target.campaign.days.map((_,index)=>option(index,`第 ${index+1} 天`)),dayIndex);$('admin-day-duration').value=day.duration;$('admin-day-income').value=day.maxIncome;$('admin-day-star-satisfaction').value=dayStars.satisfaction;$('admin-day-star-cost').value=dayStars.efficiency.maxCost;$('admin-day-star-queue').value=dayStars.efficiency.maxQueue;}
