(() => {
  ...
  let catalog = null, chapterIndex = 0, levelIndex = 0, routeIndex = 0, dayIndex = 0, regionIndex = 0;
  ...
  function render() {
    ...
    const stars=target.starTargets||{satisfaction:80,efficiency:{maxCost:target.budget,maxQueue:20}};
    $('admin-star-satisfaction').value=stars.satisfaction;$('admin-star-cost').value=stars.efficiency.maxCost;$('admin-star-queue').value=stars.efficiency.maxQueue;
    ...
    if(target.campaign){
      const regionList=target.campaign.regions||[];dayIndex=Math.min(dayIndex,target.campaign.days.length-1);regionIndex=Math.min(regionIndex,Math.max(0,regionList.length-1));
      fillSelect($('admin-region'),regionList.length?regionList.map((item,index)=>option(index,item.name)): [option('','尚无扩建区域')],regionIndex);
      const region=regionList[regionIndex];$('admin-region-fields').hidden=!region;
      if(region){$('admin-region-name').value=region.name;$('admin-region-cost').value=region.cost;for(const field of ['day','delivered','income','satisfaction'])$('admin-region-'+field).value=region.unlock?.[field]??(field==='day'?1:0);$('admin-region-cells').textContent=`已划入 ${region.cells.length} 格：${region.cells.slice(0,10).join('、')}${region.cells.length>10?'…':''}`;}
    }
  ...
  function startEditor() {
    ...
    $('admin-apply-size').onclick=()=>{const target=level(),oldWidth=target.width||16,oldHeight=target.height||12,width=Number($('admin-width').value),height=Number($('admin-height').value);if(!Number.isInteger(width)||!Number.isInteger(height)||width<8||width>64||height<8||height>64)return setStatus('地图宽高必须是 8 至 64 的整数','invalid');if(width===oldWidth&&height===oldHeight)return;const dx=Math.floor(width/2)-Math.floor(oldWidth/2),dy=Math.floor((height-1)/2)-Math.floor((oldHeight-1)/2),translate=cell=>{const x=cell%oldWidth+dx,y=Math.floor(cell/oldWidth)+dy;return x>=0&&x<width&&y>=0&&y<height?y*width+x:null;};const routeSets=[target.routes,...(target.campaign?.routes?[target.campaign.routes]:[])],buildings=routeSets.flatMap(items=>items.flatMap(item=>[...item.homes,...item.goals]));if(buildings.some(item=>translate(item.cell)===null))return setStatus('新边界会裁掉建筑，请先移动建筑','invalid');if(!confirm(`将地图从 ${oldWidth} × ${oldHeight} 调整为 ${width} × ${height}？边界外地形和道路会被裁掉。`))return;commit(()=>{const mapCells=values=>values.map(translate).filter(cell=>cell!==null);target.water=mapCells(target.water);target.bridges=mapCells(target.bridges);target.trees=mapCells(target.trees);target.initialRoads=(target.initialRoads||[]).map(road=>({...road,cell:translate(road.cell)})).filter(road=>road.cell!==null);target.initialEdges=(target.initialEdges||[]).map(([a,b])=>[translate(a),translate(b)]).filter(([a,b])=>a!==null&&b!==null);for(const region of target.campaign?.regions||[])region.cells=mapCells(region.cells);for(const building of buildings)building.cell=translate(building.cell);target.width=width;target.height=height;});};
    $('admin-campaign-enabled').onchange=()=>commit(()=>{const target=level();if($('admin-campaign-enabled').checked){delete target.referenceDesign;target.campaign={days:Array.from({length:5},(_,i)=>({duration:target.duration,maxIncome:18+i*2,starTargets:clone(target.starTargets||{satisfaction:80,efficiency:{maxCost:target.budget,maxQueue:20}})})),routes:clone(target.routes),regions:[]};for(const item of target.campaign.routes)for(const building of [...item.homes,...item.goals]){building.unlock={day:1};building.upgrades=[];}}else{target.routes=clone(target.campaign.routes);delete target.campaign;}dayIndex=regionIndex=routeIndex=0;});
    $('admin-campaign-day').onchange=()=>{dayIndex=Number($('admin-campaign-day').value);render();};
    $('admin-region').onchange=()=>{regionIndex=Number($('admin-region').value)||0;render();};
    for(const [id,property,numeric] of [['admin-region-name','name'],['admin-region-cost','cost',true]])$(id).onchange=()=>commit(()=>level().campaign.regions[regionIndex][property]=numeric?Number($(id).value):$(id).value,{preview:false,render:false});
    for(const field of ['day','delivered','income','satisfaction'])$('admin-region-'+field).onchange=()=>commit(()=>{const condition=level().campaign.regions[regionIndex].unlock,value=Number($('admin-region-'+field).value);if(field!=='day'&&value===0)delete condition[field];else condition[field]=value;},{preview:false,render:false});
    $('admin-add-region').onclick=()=>{const cells=selectedCells();if(!cells.length)return setStatus('请先在地图中选择扩建区域格子','invalid');const regions=level().campaign.regions||(level().campaign.regions=[]),occupied=new Set(regions.flatMap(region=>region.cells));if(cells.some(cell=>occupied.has(cell)))return setStatus('所选格子与已有扩建区域重叠','invalid');commit(()=>{const id=uniqueId('expansion',regions);regions.push({id,name:`扩建区域 ${regions.length+1}`,cells:[...cells].sort((a,b)=>a-b),cost:3,unlock:{day:Math.min(level().campaign.days.length,dayIndex+1)}});regionIndex=regions.length-1;});};
    $('admin-region-use-selection').onclick=()=>{const cells=selectedCells(),regions=level().campaign.regions,region=regions[regionIndex];if(!region||!cells.length)return setStatus('请先选择区域和地图格子','invalid');const occupied=new Set(regions.filter(item=>item!==region).flatMap(item=>item.cells));if(cells.some(cell=>occupied.has(cell)))return setStatus('所选格子与其他扩建区域重叠','invalid');commit(()=>region.cells=[...cells].sort((a,b)=>a-b));};
    $('admin-delete-region').onclick=()=>{const regions=level().campaign.regions,region=regions[regionIndex];if(region&&confirm(`删除扩建区域「${region.name}」？`))commit(()=>{regions.splice(regionIndex,1);regionIndex=Math.max(0,regionIndex-1);});};
    ...
    $('admin-capture-reference').onclick=()=>{
      const target=level(),design=window.TrafficGameAdmin.captureDesign();
      if(!target.campaign){commit(()=>target.referenceDesign=clone(design));setStatus('已将当前完整设计保存为参考答案草稿','valid');return;}
      const actualDay=window.TrafficGameAdmin.campaignDayIndex(),active=trial?.levelId===target.id&&trial.fingerprint===levelFingerprint(target);
