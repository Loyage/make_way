(() => {
  ...
  let savedSnapshot = '', history = [], historyIndex = -1, validationTimer = 0;
  ...
  function setStatus(text, kind = '') { const el=$('admin-validation');el.textContent=text;el.className='admin-validation '+kind; }
  async function validate() {
    setStatus('正在校验草稿…');
    try { await api('/api/validate',{method:'POST',body:JSON.stringify(catalog)});setStatus('✓ 草稿有效，可以发布','valid');return true; }
    catch(error){setStatus('✕ '+error.message,'invalid');return false;}
  }
  ...
  function commit(action, options = {}) {
    action();syncCampaign();pushHistory();dirtyLabel();render();
    if(options.preview!==false)preview(options.levelId || level()?.id);
    scheduleValidation();
  }
  function restore(index) {
    if(index<0||index>=history.length)return;historyIndex=index;catalog=JSON.parse(history[index]);
  ...
  function render() {
    ...
    $('admin-move-up').disabled=levelIndex===0;$('admin-move-down').disabled=levelIndex===(chapter()?.levels.length||1)-1;$('admin-delete-level').disabled=allLevels().length<=1;
  }
  function bindField(id,property,numeric=false){$(id).onchange=()=>commit(()=>{level()[property]=numeric?Number($(id).value):$(id).value;});}

  function startEditor() {
    catalog=window.TrafficAdminCatalog;savedSnapshot=snapshot();history=[savedSnapshot];historyIndex=0;updateHistory();dirtyLabel();render();scheduleValidation();
    ...
    $('admin-apply-size').onclick=()=>{const target=level(),oldWidth=target.width||16,oldHeight=target.height||12,width=Number($('admin-width').value),height=Number($('admin-height').value);if(!Number.isInteger(width)||!Number.isInteger(height)||width<8||width>64||height<8||height>64)return setStatus('地图宽高必须是 8 至 64 的整数','invalid');if(width===oldWidth&&height===oldHeight)return;const dx=Math.floor(width/2)-Math.floor(oldWidth/2),dy=Math.floor((height-1)/2)-Math.floor((oldHeight-1)/2),translate=cell=>{const x=cell%oldWidth+dx,y=Math.floor(cell/oldWidth)+dy;return x>=0&&x<width&&y>=0&&y<height?y*width+x:null;};const routeSets=[target.routes,...(target.campaign?.routes?[target.campaign.routes]:[])],buildings=routeSets.flatMap(items=>items.flatMap(item=>[...item.homes,...item.goals]));if(buildings.some(item=>translate(item.cell)===null))return setStatus('新边界会裁掉建筑，请先移动建筑','invalid');if(!confirm(`将地图从 ${oldWidth} × ${oldHeight} 调整为 ${width} × ${height}？边界外地形和道路会被裁掉。`))return;commit(()=>{const mapCells=values=>values.map(translate).filter(cell=>cell!==null);target.water=mapCells(target.water);target.bridges=mapCells(target.bridges);target.trees=mapCells(target.trees);target.initialEdges=(target.initialEdges||[]).map(([a,b,grade])=>[translate(a),translate(b),grade]).filter(([a,b])=>a!==null&&b!==null);for(const building of buildings)building.cell=translate(building.cell);target.width=width;target.height=height;});};
    $('admin-campaign-enabled').onchange=()=>commit(()=>{const target=level();if($('admin-campaign-enabled').checked){target.campaign={days:Array.from({length:5},(_,i)=>({duration:target.duration,maxIncome:18+i*2})),routes:clone(target.routes)};for(const item of target.campaign.routes)for(const building of [...item.homes,...item.goals]){building.unlock={day:1};building.upgrades=[];}}else{target.routes=clone(target.campaign.routes);delete target.campaign;}dayIndex=0;routeIndex=0;});
    ...
    $('admin-capture-roads').onclick=()=>{const design=window.TrafficGameAdmin.captureDesign(),grades=new Map(design.roads.map(item=>[item.cell,item.grade]));commit(()=>{level().initialEdges=design.edges.map(([a,b])=>[a,b,Math.max(grades.get(a)||0,grades.get(b)||0)]);});setStatus('已将当前道路、等级和显式连接写入关卡草稿；道路引导、信号和公交不属于初始关卡定义','valid');};
    $('admin-undo').onclick=()=>restore(historyIndex-1);$('admin-redo').onclick=()=>restore(historyIndex+1);
    $('admin-publish').onclick=async()=>{if(!await validate())return;try{await api('/api/levels',{method:'PUT',body:JSON.stringify(catalog)});savedSnapshot=snapshot();history=[savedSnapshot];historyIndex=0;updateHistory();dirtyLabel('已发布到关卡文件');setStatus('✓ 发布成功；管理员预览已是最新草稿，公开游戏服务重启后生效','valid');}catch(error){setStatus('发布失败：'+error.message,'invalid');}};
    $('admin-reload').onclick=async()=>{if(snapshot()!==savedSnapshot&&!confirm('放弃所有未发布草稿并重新载入？'))return;try{catalog=clone((await api('/api/levels')).catalog);savedSnapshot=snapshot();history=[savedSnapshot];historyIndex=0;chapterIndex=levelIndex=routeIndex=dayIndex=0;render();preview();dirtyLabel();updateHistory();scheduleValidation();}catch(error){setStatus(error.message,'invalid');}};
