...
async function main() {
  ...
  try {
    ...
    await evaluate('TrafficGameAdmin.selectCell(0)');await click('#build-road');assert.equal(await text('budget'),'35','challenge design exists before sandbox copy');
    await click('#sandbox-mode');assert.equal(await evaluate('document.querySelector("#mode-dialog").open'),true);
    await evaluate('document.querySelector("#sandbox-entry-budget").value="unlimited"');await click('#confirm-mode');assert.equal(await text('timer'),'∞');assert.equal(await text('budget'),'∞');assert.equal(await evaluate('document.querySelector("#sandbox-mode").getAttribute("aria-pressed")'),'true');
    assert.equal(await evaluate('TrafficGameAdmin.captureDesign().roads.some(road=>road.cell===0)'),true,'sandbox copies the current challenge design');
    await evaluate('const select=document.querySelector("#sandbox-demand");select.value="2";select.dispatchEvent(new Event("change"))');assert.equal(await evaluate('document.querySelector("#sandbox-demand").value'),'2');
    await click('#sandbox-continuous');assert.equal(await evaluate('document.querySelector("#sandbox-continuous").checked'),true);
    ...
    await evaluate('TrafficGameAdmin.selectCell(0)');await click('#remove-road');assert.equal(await evaluate('TrafficGameAdmin.captureDesign().roads.some(road=>road.cell===0)'),false,'sandbox can alter its copied design');
    assert.ok((await text('board-status')).includes('可实时修改'));await click('#reset-traffic');assert.ok((await text('board-status')).includes('沙盒规划'));
    await click('#challenge-mode');await click('#confirm-mode');assert.notEqual(await text('timer'),'∞');
    assert.equal(await evaluate('TrafficGameAdmin.captureDesign().roads.some(road=>road.cell===0)'),true,'returning restores the untouched challenge design');
    assert.equal(await text('budget'),'35');assert.equal(await evaluate('document.querySelector("#challenge-mode").getAttribute("aria-pressed")'),'true');
    await evaluate('TrafficGameAdmin.selectCell(0)');await click('#remove-road');assert.equal(await text('budget'),'36','challenge fixture is restored for later smoke checks');
