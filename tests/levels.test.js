...
for (const level of LEVELS) {
  test(`${level.name}: valid independent terrain and dynamic route counts`, () => {
    ...
    assert.equal(city.queues.length,level.routes.length);assert.equal(city.remaining,level.budget-[...city.roads].reduce((sum,n)=>sum+(city.bridges.has(n)?0:1)+(city.roadGrades.get(n)||0),0));
...
test('six lessons unlock tools progressively and definitions stay immutable', () => {
  assert.deepEqual(LEVELS.map(l=>l.id),['neighborhood','demolition-school','avenue-school','woodland','signal-school','rush-hour']);
