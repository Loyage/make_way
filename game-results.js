...
(function (root) {
  ...
  function commuteReport(times) {
    ...
    const score = valid.length ? Math.round(bands.reduce((sum, band) => sum + band.count * band.satisfaction, 0) / valid.length) : 0;
    const average = valid.length ? valid.reduce((sum, time) => sum + time, 0) / valid.length : 0;
    return { count: valid.length, score, average, bands };
  }
